#!/usr/bin/env python3
"""
SFH-SGP_NEURAL_REPRESENTATION_GEOMETRY_01 - FAST VERSION
"""
import numpy as np
import pandas as pd
import torch
...
X_train = train_ds.data.float().view(-1, 784).numpy()[:5000] / 255.0
y_train = train_ds.targets.numpy()[:5000]
...
for model_name in models_config.keys():
    ...
    alpha_by_layer = model_df.groupby('layer_idx')['alpha'].mean().values
    det_by_layer = model_df.groupby('layer_idx')['det'].mean().values
    if len(alpha_by_layer) > 1:
        alpha_gradient = np.mean(np.gradient(alpha_by_layer))
        det_gradient = np.mean(np.gradient(det_by_layer))
        print(f"{model_name}: α gradient={alpha_gradient:.4f}, DET gradient={det_gradient:.4f}")
...
print(f"3. Operator geometry shift (α std): {op_shift:.4f}")
