...
def sim_kuramoto(N, K, A, seed):
    ...
    omega = rng_local.normal(0, SIGMA, N)
...
for N in N_VALUES:
    ...
    for idx, K in enumerate(K_VALUES):
        ...
        mean_R, chi, binder = compute_metrics(R_trials)
        mean_R_curve.append(mean_R)
        chi_curve.append(chi)
        binder_curve.append(binder)
    results[N] = {
        "R": np.array(mean_R_curve),
...
plt.legend()
plt.grid(True)
plt.savefig("phase252_script4_order_parameter.png", dpi=200, bbox_inches="tight")
plt.close()
...
plt.title("Susceptibility Peak Growth")
plt.legend()
plt.grid(True)
plt.savefig("phase252_script4_susceptibility.png", dpi=200, bbox_inches="tight")
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig("phase252_script4_binder.png", dpi=200, bbox_inches="tight")
plt.close()
