...
print("\n" + "=" * 70)
print("STEP 2: REGIME MAPPING")
print("=" * 70)

print("\nTask → Regime mapping:")
for t, r in TASK_TO_REGIME.items():
    print(f"  {t:12s} → {r}")
...
for subj in subj_list[:3]:
    ...
    for task in TASKS:
        ...
        regime = row['regime']
        print(f"  {task:12s} ({regime:8s}): [{vec[0]:.3f}, {vec[1]:.3f}, {vec[2]:.3f}, {vec[3]:.3f}]")
...
chance_level = 1.0 / len(REGIMES)
print(f"\nChance level: {chance_level:.2f} (1/{len(REGIMES)})")
...
print("\nConfusion Matrix:")
cm = confusion_matrix(y, y_pred, labels=REGIMES)
print(f"\n{'Regime':<12}", end="")
for r in REGIMES:
    print(f" {r[:6]:>6}", end="")
print()
...
for r in REGIMES:
    mask = y == r
    reg_acc = accuracy_score(y[mask], y_pred[mask])
    print(f"  {r:12s}: {reg_acc:.4f}")
...
if accuracy > chance_level + 0.1:
    print(f"\nRESULT: accuracy = {accuracy:.2f} >> chance {chance_level:.2f}")
    print("Dimensionality encodes REGIME information")
else:
    print(f"\nRESULT: accuracy ≈ chance")
    print("Dimensionality does NOT encode regime information")