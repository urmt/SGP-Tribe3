...
# Now test different tau values
tau_values = [1, 2, 3, 5]
print("\nTau sensitivity test for dynamical torsion (χ):")
...
for tau in tau_values:
    ...
    for node_idx in range(n_nodes):
        ...
        if len(base) < 3*tau:  # need at least 3*tau points
            # If not enough points, we skip or adjust? For safety, we'll use the maximum possible tau for this base.
            # But in our case, base length is 100, so tau=5 -> 3*5=15, which is less than 100, so safe.
            # We'll just continue if not enough, but we know it's safe.
            continue
    ...
    chi_nodes = chi_nodes / chi_nodes.max() if chi_nodes.max() > 0 else chi_nodes
...
print("\nNote: Correlation should remain stable (similar magnitude and same sign) across taus.")